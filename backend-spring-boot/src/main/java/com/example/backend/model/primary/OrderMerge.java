    ...
    public static Order mergeItems(Order order, Order persistedOrder) {
		...
		for (Item item : itemList) {
			...
			//Optional<Item> optionalCopyOfPersistedItem = persistedItemList
			    .stream()
			    .filter(copyOfPersistedItem -> copyOfPersistedItem.getId().equals(item.getId()))
			    .findFirst();
			
			if (optionalCopyOfPersistedItem.isPresent()) {
				
				persistedOrder.getItemList().add(ItemMerge.mergeSimpleAttributes(item, optionalCopyOfPersistedItem.get()));
